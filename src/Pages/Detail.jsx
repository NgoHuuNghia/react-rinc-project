...
const GameDetail = () => {
    ...
    if(loading === false || !detail || !screenshots){
        return <h2>Loading page</h2>
    }
    else {
        const {name, description, genres, background_image} = detail
                                ...
                                {genres
                                    .map((item, index, array) =>{
                                        if(index === array.length - 1){
                                            return <div key={item.id}><a  href='/'>{item.name}</a></div>
                                        } else return <div key={item.id}><a href='/'>{item.name}</a><FaChevronRight /> </div>
                                    }
                                )}
                            </div>
